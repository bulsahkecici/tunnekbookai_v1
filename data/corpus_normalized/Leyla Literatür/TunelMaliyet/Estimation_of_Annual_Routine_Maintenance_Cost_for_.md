...
where Cit is the TMC of road i for year t (CNY/lane/m); TCit is the TMC of road i for year t (RMB); n ik is the number of lanes of road i for lane k ; the number of tunnel lanes may be diferent in diferent sections of a road, k [UNRESOLVED_CHAR] 2,4,6; and L ik is the tunnel length of the road i for lane k (m).
...
where x 2015 should belong to ( -12.80, 39.36), while x2015 =64.29, x2015 is the outlier. Delete the data and take the average value of the others ( x [UNRESOLVED_CHAR] 13 . 28) to make up the gap.
...
where yit is equal to ln Cit ; α 0 is the constant term; u k is the district efect ( k [UNRESOLVED_CHAR] 1, 2, 3 represent Shanbei, Guanzhong and Shannan, respectively); β is the vector of estimated coefcients ( β 1 , β 2 , · · · , β m ) for the number of variables, m ; and x it ′ is the ( x 1 it , x 2 it , · · · , x mit ) ′ , m × 1 transpose vector.
