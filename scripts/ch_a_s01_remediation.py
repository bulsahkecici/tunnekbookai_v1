#!/usr/bin/env python3
"""Prepare a local-only CH-A-S01 evidence remediation packet.
...
RELATED_IDS = (SECTION_ID, SECTION_TITLE, "RQ-001-01", "EP-001")
SEARCH_QUERIES = (
    "tünel tanımı", "tünel nedir", "tünelin tanımı", "yeraltı yapısı tünel",
    "karayolu tüneli tanımı", "tunnel definition", "definition of tunnel",
    "underground tunnel structure", "underground passage tunnel",
    "road tunnel definition", "transportation tunnel definition",
)
...
LOG = logging.getLogger("ch_a_s01_remediation")
...
def load_section_state(artifacts: list[Path], root: Path) -> dict[str, Any]:
    ...
    for path in artifacts:
        ...
        for document in documents:
            for record in _walk(document):
                ...
                if SECTION_ID in rendered or "RQ-001-01" in rendered or "EP-001" in rendered:
    ...
    return {
        ...
        "section_scope": _first(records, ("section_scope", "scope")),
        "research_question": _first(records, ("research_question", "research_question_text", "question")),
...
def _classification_system_prompt() -> str:
    return (
        "Classify atomic claims for CH-A-S01 — Tünelin Tanımı only. Use only the supplied "
        "candidate text; use no pretrained knowledge or external facts. Do not strengthen, "
        "combine, repair, or infer claims. Topical similarity is not support. Exclude later-section "
        "topics such as classifications, history, NATM, TBM, support, detailed geotechnics, cost, "
        "maintenance, fire, and accidents. Return JSON only as {\"claims\": [...]}. Each claim must "
        "contain claim_text, document_id, source_relative_path, locator, scope, qualifier, source_type, "
        "authority_state, support_status, section_applicability, disposition, reason. disposition must "
        f"be one of {sorted(DISPOSITIONS)}. Preserve qualifiers."
    )
...
def classify_claims_local(candidates: list[dict[str, Any]], model: str, api_base: str) -> list[dict[str, Any]]:
    smoke_test_local_llm(model, api_base)
    claims: list[dict[str, Any]] = []
    for candidate in candidates:
        parsed = _native_chat_json(model, api_base, _classification_system_prompt(), _llm_prompt(candidate))
        model_claims = parsed.get("claims")
        if not isinstance(model_claims, list):
            raise RemediationError("LOCAL_LLM_CLASSIFICATION_FAILED: model response has no claims list")
        for claim in model_claims:
            normalized = _validate_claim(claim, candidate)
...
def build_writer_packet(state: dict[str, Any], claims: list[dict[str, Any]], readiness: str, generated_at: str) -> dict[str, Any] | None:
    ...
    for index, claim in enumerate(admitted, start=1):
        claim.setdefault("claim_id", f"CH-A-S01-REM-{index:03d}")
    citation_mapping = [{
    ...
    return {
        ...
        "generated_at": generated_at, "generator": "scripts/ch_a_s01_remediation.py",
...
def _report(summary: dict[str, Any], state: dict[str, Any], outputs: list[str]) -> str:
    return "\n".join([
        "# CH-A-S01 Remediation Report", "", f"- Generated (UTC): {summary['generated_at']}",
...
def write_artifacts(output_dir: Path, stamp: str, candidates: list[dict[str, Any]], packet: dict[str, Any] | None,
    ...
    paths = {
        "candidates": versioned_path(output_dir, "ch_a_s01_candidates.jsonl", stamp),
        "packet": versioned_path(output_dir, "ch_a_s01_writer_evidence_packet.json", stamp) if packet else None,
        "report": versioned_path(output_dir, "ch_a_s01_remediation_report.md", stamp),
        "manifest": versioned_path(output_dir, "ch_a_s01_remediation_manifest.json", stamp),
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        manifest = {
            ...
            "generator": "scripts/ch_a_s01_remediation.py", "privacy_mode": "LOCAL_ONLY", "summary": summary,
        ...
        output_dir = root / "data" / "book" / "remediation" / "ch_a_s01"
        if not args.dry_run:
